...
| Version | What |
|---------|------|
| v=117 / sw v4 | **Field QA (mobile + design + day-one red team, 5 readers): 2 blockers, 15 majors — fixed.** BLOCKERS: (1) the client signature pad froze its bitmap at first open — after an orientation change ink landed ~1.5x off the finger and half the pad was dead, and that mangled image was saved as the client's signature; now sized per device-pixel-ratio, re-sized on rotate with a plain "the screen turned — please sign again", coordinates mapped live. (2) a failed profile read at sign-in defaulted the role to Engineer — a technician would have lost My Work entirely; now it falls back only to what this device last read for this account, else shows a retry screen. (3) a queued offline Start Work removed the only button to record the completion (the flush layer was already built for the pair); the row now keeps its action. MAJORS: outbox chip no longer floats over an open form (z-15) and refuses to replace a half-filled completion; photo-lookup failure says "could not be loaded — try again" instead of "No photos attached." (which made engineers send good work back); client-signature failures are network-aware and treat a lost reply on an already-signed report as success; offline completion replay applies the same date/name patch as the online path; all six jsPDF entry points say so when the CDN script is missing; queued work now retries on focus/10-min tick, not only on an 'online' event that dead plant Wi-Fi never fires; blanket 42/44px phone touch targets (the old rule missed every screen shipped since v108, incl. appAlert/appConfirm and the modal close button); info dots got a 40px hit area, a real tap toggle (they relied on emulated hover), darker border, and tooltips keep their line breaks; contrast fixes — green sign/approve buttons 3.30→5.02:1, signature timestamps/WO numbers/report hash 2.45→4.55:1, attention-chip bubbles 3.19/2.56→4.93:1/10.6:1; attention chip labels shortened so the strip no longer pushes the first table below the fold; section headers and card action rows wrap on phones; card stacks got a gutter. SHELL: service-worker navigations bypass the HTTP cache (index.html carries no ?v=, so a host cache window could serve yesterday's shell), cache bumped to mm-shell-v4. Also PPM_BASELINE moved to 2026-09-01 so the handover does not open on a pile of build-period "overdue PPM" |
| v=116 / SQL 55 | The whole-code audit (6 readers, 336 functions checked; 3 majors, 14 minors, 7 cosmetics — all fixed or deliberately annotated): (1) v112's function-name collision had silently killed Team → Generate Schedule — renamed and revived; (2) per-plant Notifications settings had lost their only button — restored on Plants rows; (3) SQL 55 hardens profiles (client INSERTs refused, deleting an Admin needs the Superadmin, Superadmin self-demotion blocked), enforces amendment scope server-side (content re-listing signed jobs refused; compiling over an in-flight technician report refused), and makes open-job quick-completes record completion-day work + the completer's name (offline replays now match online); (4) the daily digest is hold- and plan-aware (held jobs are no longer emailed as overdue forever; far-future plans stop the every-day email) — REDEPLOY send-notifications; (5) hold-aware To start rows/counts/bell; future plans read "starts in Nd"/"Scheduled", never negative-day "ongoing"; nav badge includes ready visits; "N open" dropdown counts include returned jobs; report caches re-hydrate before compile offers; issues raised between submit and client-sign no longer vanish (freeze-moment fix) and issue dates match IST; offline start replay refuses reassigned jobs and yields to a queued completion; My Work shows orphan reports needing action beyond the visit caps; compiled PDFs say "Submitted", not "Signed", under the technician; label sweep (visit summaries everywhere, Fix & send again, honest modal titles); PDF preview header escapes plant names; log export gets the house header. Harness: amendment fixture now asserts the refusal + clean creation (43 asserts). SQL 55 executed by Mihir 2026-08-31 (all six verify rows true); send-notifications redeployed by Mihir 2026-08-31. His harness re-run hit a stale SQL-editor snippet (42 asserts, pre-v116 copy) — full pass of that copy; the current file holds 43, and the new refusal test needed a fixture fix (the signed report's content now carries real job ids so covered_job_ids has teeth). **Re-run by Mihir 2026-08-31 from the current file: 43/43 PASS, refusal line witnessed** |
| v=115 / SQL 54 | Engineer-own work joins the co-signed flow: taking or completing an unassigned job stamps you as the worker (SQL 54), your own finished visit is offered as a Create & sign report (and waits in Visits ready for a report), and self-reports carry an honest signature pair — "Work done & signed by <name> (Engineer)" + client — in the view and the PDF. Harness +3 asserts (section 13). SQL 54 executed by Mihir 2026-08-31; **harness run: full pass** (42 asserts at that point — my "44/44" label was a miscount) |
| v=114 | Report reckoning + polish, from Mihir's questions: the pre-technician per-job "Confirm & Generate Service Report" button and its 100-line generator DELETED (one official document now — the co-signed Service Report); the two remaining unsigned exports renamed honestly (Maintenance Log → "Maintenance Summary" via a Summary PDF button; engineer-own visits → "Visit Summary") so no unsigned PDF shares a name with the signed record; signed-report PDF now opens the in-app preview instead of a blind download; info dots became floating hover/tap tooltips (no modal); engineer nav reordered (Engineering Corner → Maintenance Log → Equipment → Team); tour + guides updated |
